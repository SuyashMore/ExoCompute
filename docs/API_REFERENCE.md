...
PiEstimationUnit.Input(
    num_samples: int
)

PiEstimationUnit.Output(
    pi_estimate: float,
    samples_inside: int
)
```
...
##### RandomWalkUnit
```python
RandomWalkUnit.Input(
    num_steps: int,
    step_size: float
)

RandomWalkUnit.Output(
...
GaussianBlurUnit.Input(
    image_data: bytes,  # Base64 encoded
    kernel_size: int
)

GaussianBlurUnit.Output(
    processed_image: bytes
)
```

##### EdgeDetectionUnit, ImageRotateUnit, HistogramEqualizationUnit

Similar structure for other image processing operations.
